...
namespace WintapCoreSvcMgr.Database
{
    ...
    public class BackupDatabaseManager : IDisposable
    {
        ...
        //private readonly ILogger<BackupDatabaseManager> _logger;
        ...
        public async Task<DatabaseOperationResult> ProcessMiniTraceETL()
        {
            ...
            try
            {
                ...
                {
                    LogWarning($"Mini-trace ETL file not found: {MINI_TRACE_ETL_PATH}");
                ...
                return DatabaseOperationResult.DBSuccess();
            }
            catch (Exception ex)
            {
        ...
        public DatabaseOperationResult SynchronizeDatabases()
        {
            try
            {
                ...
                {
                    LogError($"Backup database not found: {BACKUP_DB_PATH}");
                ...
                File.Copy(BACKUP_DB_PATH, MAIN_DB_PATH, overwrite: true);

                // Copy WAL file if it exists
                var backupWalPath = BACKUP_DB_PATH + ".wal";
        ...
        public BackupDatabaseStatus GetBackupDatabaseStatus()
        {
            try
            {
                ...
                return new BackupDatabaseStatus
                {
                    ...
                    DatabaseExists = File.Exists(BACKUP_DB_PATH),
                    DatabasePath = BACKUP_DB_PATH,
                    ...
                    LastModified = File.Exists(BACKUP_DB_PATH) ? File.GetLastWriteTime(BACKUP_DB_PATH) : null,
                    CheckedAt = DateTime.UtcNow,
        ...
        {
           //_logger?.LogInformation($"[BackupDatabaseManager] {message}");
        }

        private void LogWarning(string message)
        {
            //_logger?.LogWarning($"[BackupDatabaseManager] {message}");
        }

        private void LogError(string message)
        {
            //_logger?.LogError($"[BackupDatabaseManager] {message}");
        }
