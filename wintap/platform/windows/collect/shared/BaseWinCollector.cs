    ...
    public class BaseWinCollector : BaseCollector
    {
        ...
        internal void CacheStatistics()
        {
            try
            {
                ...
                {
                    RegistryKey providerKey = Registry.LocalMachine.CreateSubKey(Strings.RegistryCollectorPath + "\\" + CollectorName);
        ...
        private void Averager_Elapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                if (!Watchdog.PerformanceBreach)
                {
                    ...
                    lastAveraged = DateTime.Now;
                    WintapLogger.Log.Append(CollectorName + ", total events over 10/sec: " + Counter, LogLevel.Debug);
                    if (Properties.Settings.Default.Profile != "Developer")  // don't throttle providers if running in dev mode.
                    {
                        RegistryKey wintapKey = Registry.LocalMachine.CreateSubKey(Strings.RegistryCollectorPath + "\\" + CollectorName);
