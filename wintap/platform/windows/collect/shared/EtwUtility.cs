...
namespace gov.llnl.wintap.platform.windows.collect.shared
{
    ...
    public class ETWProvider
    {
        ...
        internal void Stop()
        {
            ...
            try
            {
                userModeProvider.StopProcessing();
                userModeProvider.Dispose();
                userModeSession.Stop();
                userModeSession.Dispose();
            }
            catch (Exception ex) { }
        }
        ...
        private void EtwWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                userModeSession = new TraceEventSession("ETWScanner." + ProviderName, TraceEventSessionOptions.Create);
                userModeSession.BufferSizeMB = 250;
                userModeSession.EnableProvider(ProviderGuid);
                userModeProvider = new ETWTraceEventSource("ETWScanner." + ProviderName, TraceEventSourceType.Session);
        ...
        {
            if (obj.ProviderGuid == ProviderGuid)
            {
                ETWSample sample = new ETWSample() { EventName = obj.EventName, ProviderName = ProviderName, SampleId = Guid.NewGuid().ToString() };
