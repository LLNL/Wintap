...
namespace gov.llnl.wintap.platform.windows.collect.etw.helpers
{
    abstract internal class BaseEvent
    {
        ...
        public BaseEvent(TraceEvent obj)
        {
            ...
            EventArray = dted.Split(new char[] { '\"' });
            PID = Convert.ToInt32(EventArray[3]);
        }
    }
    ...
    internal class RegKeyEvent : BaseEvent
    {
        public RegKeyEvent(TraceEvent obj) : base(obj)
        {
            ...
            KeyObject = ulong.Parse(EventArray[15].Replace("0x", ""), System.Globalization.NumberStyles.HexNumber);
            BaseObject = ulong.Parse(EventArray[13].Replace("0x", ""), System.Globalization.NumberStyles.HexNumber);
            Path.Append(EventArray[23].ToLower().Trim(new char[] { '\\' }));
        ...
        {
            Path.Clear();
            Path.Append(regParents[BaseObject]);
            Path.Append(@"\");
            Path.Append(EventArray[23].ToLower().Trim(new char[] { '\\' }));
        }
    }
    ...
    class RegSetValueEvent : BaseEvent
    {
        ...
        {
            Handle = (ulong)obj.PayloadByName("KeyObject");
            Name = (string)obj.PayloadByName("ValueName");
            string keyname = (string)obj.PayloadByName("KeyName");
            DataType = (int)obj.PayloadByName("Type");
        }
