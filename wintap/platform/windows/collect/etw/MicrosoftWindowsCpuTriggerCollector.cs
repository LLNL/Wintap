...
namespace gov.llnl.wintap.platform.windows.collect.etw
{
    internal class MicrosoftWindowsCpuTriggerCollector : EtwProviderCollector
    {
        public MicrosoftWindowsCpuTriggerCollector() : base()
        {
            ...
            CollectorName = "Microsoft.Windows.CpuTrigger";
            ...
            try
            {
                if (obj.EventName == "HighCpuUsageEvent")
                {
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        metric.PID = pid;
                        CollectorName = GetType().Name; // resetting to the classname so that dynamic consumers can instantiate the model object
