...
namespace gov.llnl.wintap.platform.windows.collect.shared
{
    ...
    /// </summary>
    internal abstract class EtwProviderCollector : BaseWindowsSensor
    {
        ...
        public BaseWindowsSensor.CollectorTypeEnum CollectorType;
        ...
        {
            CollectorType = BaseWindowsSensor.CollectorTypeEnum.ETW;
        ...
        private void etwListenerThread_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                WintapLogger.Log.Append("Starting ETW consumer on: " + SensorName + ", privider id: " + EtwProviderId, LogLevel.Info);
                traceEventSource.Process();  // this is a blocking call! 
                WintapLogger.Log.Append("CRITICAL: etw listening thread has stopped for: " + SensorName, LogLevel.Info);
            }
            catch (Exception ex)
            {
