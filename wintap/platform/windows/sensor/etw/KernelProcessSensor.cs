...
namespace gov.llnl.wintap.platform.windows.collect.etw
{
    ...
    /// </summary>
    internal class KernelProcessSensor : EtwProviderCollector
    {
        private Dictionary<int, string> processLookup;

        public KernelProcessSensor() : base()
        {
            SensorName = "Microsoft-Windows-Kernel-Process";
        ...
        private void parseUserModeProcessStop(TraceEvent obj)
        {
            try
            {
                ...
                long cpuCycleCount = Convert.ToInt64(obj.PayloadStringByName("CPUCycleCount").Replace(",", ""));
