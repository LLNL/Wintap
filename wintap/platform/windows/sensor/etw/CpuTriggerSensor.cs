...
{
    internal class CpuTriggerSensor : EtwProviderCollector
    {
        public CpuTriggerSensor() : base()
        {
            SensorName = "Microsoft.Windows.CpuTrigger";
            // this is the ETW Provider GUID, this what gets wired up with ETW
            EtwProviderId = "635d9d84-4106-4f3a-a5c2-7fda784ae6fc";
        }
        ...
        public override void Process_Event(TraceEvent obj)
        {
            ...
            try
            {
                if (obj.EventName == "HighCpuUsageEvent")
                {
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        metric.PID = pid;
                        SensorName = GetType().Name; // resetting to the classname so that dynamic consumers can instantiate the model object
                        ...
                        WintapMessage wintapMsg = new WintapMessage(obj.TimeStamp, metric.PID, WintapMessage.MessageTypeEnum.CpuTrigger);
                        wintapMsg.ActivityType = ActivityTypeEnum.HighCpuUsage;
                        wintapMsg.CpuTrigger = metric;
            ...
            {
                WintapLogger.Log.Append("Error processing " + SensorName + " event: " + ex.Message, LogLevel.Debug);
            }
