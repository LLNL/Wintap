...
namespace gov.llnl.wintap.platform.windows.collect.etw
{
    ...
    /// </summary>
    internal class UISensor : EtwProviderCollector
    {

        public UISensor() : base()
        {
            SensorName = "Microsoft-Windows-Win32k";
            EtwProviderId = "8C416C79-D49B-4F01-A467-E56D3AA8234C";
            TraceEventFlags = 67510272;
        ...
        private void parseFocusChange(TraceEvent obj)
        {
            try
            {
                WintapMessage msg = new WintapMessage(obj.TimeStamp, obj.ProcessID, WintapMessage.MessageTypeEnum.UI);
                msg.UI = new WintapMessage.UIData();
                msg.UI.OldProcessId = Convert.ToInt32(obj.PayloadByName("OldProcessId"));
                msg.UI.SessionId = Convert.ToInt32(obj.PayloadByName("SessionId"));
        ...
        private void parseCursorWait(TraceEvent obj)
        {
            try
            {
                WintapMessage wmBuilder = new WintapMessage(obj.TimeStamp, obj.ProcessID, WintapMessage.MessageTypeEnum.UI);
                wmBuilder.UI = new WintapMessage.UIData();
                wmBuilder.ActivityType = WintapMessage.ActivityTypeEnum.Other;
                wmBuilder.UI.SessionId = Convert.ToInt32(obj.PayloadByName("SessionId"));
                wmBuilder.UI.DisplayTimeMS = Convert.ToInt32(obj.PayloadByName("DisplayTimeMs"));
